...
uint64_t Filter<fp_t>::position_hash(long long ele)
{
    return (ele % n + n) % n;
}

template <typename fp_t>
pair<int, int> SemiSortCuckooFilter<fp_t>::position_pair(uint64_t ele)
{
...
template <typename fp_t>
SemiSortCuckooFilter<fp_t>::~SemiSortCuckooFilter() 
{
    free(T); 
}

template <typename fp_t>
void SemiSortCuckooFilter<fp_t>::init_with_params(uint64_t _mc, default_random_engine &_e, long long _n, int _m, int _filled_cell, int _max_kick_steps,
...
template <typename fp_t>
void SemiSortCuckooFilter<fp_t>::operator = (SemiSortCuckooFilter const &obj)
{
    this->e = obj.e;
...
template<typename fp_t>
void SemiSortCuckooFilter<fp_t>::init(int _n, int _m, int _step, int fp_length) {
    fp_len = fp_length;
    _n += _n & 1;
...
template <typename fp_t>
void SemiSortCuckooFilter<fp_t>::clear()
{
	this -> filled_cell = 0;
...
template <typename fp_t>
fp_t SemiSortCuckooFilter<fp_t>::fingerprint(uint64_t ele)
{
...
template <typename fp_t>
void SemiSortCuckooFilter<fp_t>::get_bucket(int pos, fp_t *store)
{
    // Default : 
    //
    // Little Endian Store
...
template <typename fp_t>
inline void SemiSortCuckooFilter<fp_t>::sort_pair(fp_t &a, fp_t &b)
{
    if ((a) < (b)) swap(a, b);
}

template <typename fp_t>
void SemiSortCuckooFilter<fp_t>::set_bucket(int pos, fp_t *store)
{
...
template <typename fp_t>
inline int SemiSortCuckooFilter<fp_t>::high_bit(fp_t fp)
{
    return (fp >> (fp_len - 4)) & ((1 << 4) - 1);
}

template <typename fp_t>
inline int SemiSortCuckooFilter<fp_t>::low_bit(fp_t fp)
{
    return fp & ((1 << (fp_len - 4)) - 1);
}

template <typename fp_t>
int SemiSortCuckooFilter<fp_t>::insert_to_bucket(fp_t *store, fp_t fp)
{
...
template <typename fp_t>
int SemiSortCuckooFilter<fp_t>::insert(uint64_t ele)
{
    return 1;
}
...
template <typename fp_t>
int SemiSortCuckooFilter<fp_t>::lookup_in_bucket(int pos, fp_t fp)
{
...
template <typename fp_t>
bool SemiSortCuckooFilter<fp_t>::lookup(uint64_t ele)
{
...
template <typename fp_t>
int SemiSortCuckooFilter<fp_t>::del_in_bucket(int pos, fp_t fp)
{
    fp_t store[8];
    get_bucket(pos, store);
...
template <typename fp_t>
bool SemiSortCuckooFilter<fp_t>::del(uint64_t ele)
{
...
template <typename fp_t>
double SemiSortCuckooFilter<fp_t>::get_load_factor()
{
    return filled_cell * 1.0 / this -> n / this -> m;
}

template <typename fp_t>
double SemiSortCuckooFilter<fp_t>::get_full_bucket_factor()
{
    return full_bucket * 1.0 / this -> n;
}

template <typename fp_t>
void SemiSortCuckooFilter<fp_t>::test_bucket()
{
...
template <typename fp_t>
int VacuumFilter<fp_t>::insert(uint64_t ele)
{
